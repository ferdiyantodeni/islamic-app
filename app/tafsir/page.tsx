                        ...
                        {filteredSurah.length === 0 ? (
                            ...
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-4 my-2 sm:my-4">
                                {filteredSurah?.map((surah: SurahItemType, index: number) => (
                                    <Link href={`/tafsir/${surah?.number}`} key={index} className="flex items-center justify-between p-5 sm:p-6 bg-white rounded-lg border border-slate-200 hover:shadow-md transition-all duration-300 hover:border-emerald-300">
